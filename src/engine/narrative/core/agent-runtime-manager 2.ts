...
export class AgentRuntimeManager {
  ...
  private async enqueueRoleRun(
    ...
    const runPromise = (previous ? previous.catch(() => undefined) : Promise.resolve()).then(run);
  ...
  private async runTaskExclusive(
    ...
    const prompt = await this.buildTaskPrompt(input, policy, adapter, saveId);
    ...
    await this.maybeCompactRuntime(runtime, onTiming);
  ...
  private async maybeCompactRuntime(
    ...
    if (!runtime.shouldCompact) return;
  ...
  private async buildTaskPrompt(
    ...
    const policyContext = await buildContextByPolicyAsync(policy.role, input.gameState, policy.contextPolicy, saveId);
