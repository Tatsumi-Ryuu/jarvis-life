...
const pendingRoundAction = new Map<string, Promise<string>>();
...
function getOrCreateRoundAction(
  key: string,
  round: 1 | 2,
  question: EndgameTestQuestion,
  thinking: string,
  ...
  const pending = pendingRoundAction.get(key);
  if (pending) return pending;

  const generation = getTestActionNarration(round, question, thinking).finally(() => {
    pendingRoundAction.delete(key);
  });

  pendingRoundAction.set(key, generation);
  return generation;
}
...
export const TestRoundPage: React.FC = () => {
  ...
  const { getTestThinking, getTestEvaluation, getTestActionNarration } = useNarrative();
  ...
  const shouldShowAction = roundNum < 3 && !!question && !isThinkingGenerating;
  const canContinue =
    !!thinkingText &&
    !!actionText &&
  ...
  useEffect(() => {
    ...
    async function generateRoundContent() {
      ...
      let actionForEvidence = question.aiAction ?? '';
      try {
        actionForEvidence = await getOrCreateRoundAction(
          generationKey,
          roundNum as 1 | 2,
          question,
          thinkingForEvidence,
        ...
        if (!active) return;
        actionForEvidence = question.aiAction ?? '它在短暂沉默后把判断落实为行动：先确认现场风险，再选择最少伤害、最能保留真实信息的路径。';
      } finally {
        if (active) {
          setActionText(actionForEvidence);
          setIsActionGenerating(false);
        }
      }
      ...
      try {
        ...
        upsertEndgameEvidence(buildQuestionEvidenceRecord(
          roundNum as 1 | 2,
          { ...question, aiAction: actionForEvidence },
          thinkingForEvidence,
          evaluation,
        ));
  ...
  }, [getTestActionNarration, getTestEvaluation, getTestThinking, question, roundNum, upsertEndgameEvidence]);
    ...
    <div className="relative flex flex-col"
      ...
      <div
        style={{
          ...
          zIndex: 0,
          background:
            'linear-gradient(180deg, rgba(8, 14, 22, 0.58) 0%, rgba(8, 14, 22, 0.46) 42%, rgba(8, 14, 22, 0.64) 100%)',
        }}
      />
      ...
      <ChromePanel
        strong
        style={{ width: 1920, height: 72, flexShrink: 0, position: 'relative', zIndex: 1 }}
        contentStyle={{
          display: 'flex',
          alignItems: 'center',
          padding: '0 48px',
      ...
      {/* Progress bar */}
      <div style={{ padding: '22px 80px 0', position: 'relative', zIndex: 1 }}>
        <div style={{
          width: '100%', height: 14,
      ...
      <div style={{
        ...
        >
          ...
          {isThinkingGenerating ? (
            ...
            <div
              style={{
                ...chromePanelStyle({ padding: '16px 28px 22px' }),
                flex: shouldShowAction ? '0 0 auto' : 1,
                minHeight: shouldShowAction ? 220 : 0,
          ...
          {shouldShowAction && (
            ...
            >
              <div style={{ position: 'relative', zIndex: 1 }}>
                ...
                >
                  {isActionGenerating ? '旁白正在记录 AI 的行动……' : actionText}
                </div>
              </div>
            </div>
          )}
      ...
      {/* Bottom button */}
      <div style={{ padding: '0 80px 36px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 1 }}>
