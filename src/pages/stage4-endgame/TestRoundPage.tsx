...
export const TestRoundPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    let active = true;
    const generationKey = `${roundNum}:${activeQuestion.id}`;

    async function generateRoundContent() {
      setIsThinkingGenerating(true);
      setEvaluatorStatus('idle');
      let thinkingForEvidence = activeQuestion.aiThinking;
      try {
        ...
        if (!active) return;
        thinkingForEvidence = activeQuestion.aiThinking;
      } finally {
        if (active) {
      ...
      let actionForEvidence = activeQuestion.aiAction ?? '';
      try {
        ...
        if (!active) return;
        actionForEvidence = activeQuestion.aiAction ?? '它在短暂沉默后把判断落实为行动：先确认现场风险，再选择最少伤害、最能保留真实信息的路径。';
      } finally {
        if (active) {
      ...
      try {
        ...
        upsertEndgameEvidence(buildQuestionEvidenceRecord(
          roundNum as 1 | 2,
          { ...activeQuestion, aiAction: actionForEvidence },
          thinkingForEvidence,
          evaluation,
        ));
