...
function parseArchiveParagraphs(text: string): ArchiveParagraph[] {
  return cleanArchiveText(text)
    .split(/\n{2,}/)
    .map((paragraph) => paragraph.trim())
    .filter(Boolean)
    .map((paragraph) => ({
      text: paragraph,
      isHistoricalRecord: /^【[^】]+】/.test(paragraph),
    }));
}
...
export const ChroniclePage: React.FC = () => {
  ...
  const chronicleCache = useRef<Partial<Record<1 | 2 | 3 | 4, string>>>({});
  const chronicleRequests = useRef<Partial<Record<1 | 2 | 3 | 4, Promise<string>>>>({});
  const [chapterText, setChapterText] = useState('');
  const [isLoaded, setIsLoaded] = useState(false);

  useEffect(() => {
    const chapter = pageNum as 1 | 2 | 3 | 4;
    const cachedText = chronicleCache.current[chapter];
    if (cachedText) {
    ...
    setChapterText('');
    let active = true;
    const request = chronicleRequests.current[chapter] ?? getChronicle(chapter);
    chronicleRequests.current[chapter] = request;

    request.then((text) => {
      chronicleCache.current[chapter] = text;
      if (!active) return;
      setChapterText(text);
      setIsLoaded(true);
    }).finally(() => {
      if (chronicleRequests.current[chapter] === request) {
        delete chronicleRequests.current[chapter];
      }
    });
  ...
  const archiveParagraphs = parseArchiveParagraphs(chapterText);
        ...
        >
          ...
          >
            {isGenerating && !isLoaded ? (
              ...
              archiveParagraphs.map((paragraph, index) => (
                <p
                  key={`${index}-${paragraph.text.slice(0, 12)}`}
                  style={{
                    margin: index === 0 ? '0 0 22px' : '0 0 24px',
                    fontStyle: paragraph.isHistoricalRecord ? 'italic' : 'normal',
                    color: paragraph.isHistoricalRecord ? 'var(--color-text-muted)' : 'var(--color-text-secondary)',
                    letterSpacing: 0,
                  }}
                >
                  {paragraph.text}
